...
//  JPanel queue = new JPanel();
  ...
  void insertPanel(JPanel panel, double weightx, double weighty, int gridx, int gridy, int gridwidth, int gridheight, int fill, JLabel label, Border b, Color bkgnd) {
  ...
  PanelMain(RunTetris runTetris) {
    ...
    insertPanel(leftMost, 4.2, 1.0, 0, 0, 1, 11, GridBagConstraints.BOTH,
            new JLabel("L"), BorderFactory.createLineBorder(new Color(171, 181, 171)),
            Color.darkGray);

    insertPanel(topmost, 4.2, 0, 1, 0, 4, 1, GridBagConstraints.HORIZONTAL,
            new JLabel("T"), BorderFactory.createLineBorder(new Color(150,0,0)),
            Color.darkGray);

    insertPanel(holdPiece, 0.01, 0, 1, 1, 2, 2, GridBagConstraints.BOTH,
            new JLabel("holdPiece"), BorderFactory.createLineBorder(new Color(0,150,0)),
            Color.darkGray);

    insertPanel(subHoldPieceLeft, 0.025, 1.0, 1, 3, 1, 7, GridBagConstraints.BOTH,
            new JLabel("l"), BorderFactory.createLineBorder(new Color(0,80,0)),
            Color.darkGray);

    insertPanel( subHoldPieceRight, 0.025, 1.0, 2, 3, 1, 7, GridBagConstraints.BOTH,
            new JLabel("r"), BorderFactory.createLineBorder(new Color(0,255,0)),
            Color.darkGray);

    insertPanel(playField, 3.8, 1.0, 3, 1, 1, 9, GridBagConstraints.BOTH,
            new JLabel("playfield"), BorderFactory.createLineBorder(new Color(90,70,60)),
            Color.darkGray);

    insertPanel(queue, 0.05,0,4,2,1,5, GridBagConstraints.BOTH,
            new JLabel("queue"), BorderFactory.createLineBorder(new Color(0, 171, 171)),
            Color.darkGray
    );

    insertPanel(info, 0.05, 0.4, 4, 6, 1, 4, GridBagConstraints.BOTH,
            new JLabel("info"), BorderFactory.createLineBorder(new Color(250,180,190)),
            Color.darkGray
    );

    insertPanel(bottommost, 4.2, 0, 1, 10, 4, 1, GridBagConstraints.HORIZONTAL,
            new JLabel("B"), BorderFactory.createLineBorder(new Color(180, 70, 220)),
            Color.darkGray
    );

    insertPanel(rightmost, 4.2, 1.0, 5, 0, 1, 11, GridBagConstraints.BOTH,

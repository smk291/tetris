...
abstract class InputTests2d {
  private static void createCell(int x, int y, Blocks2d blocks2d) {
    blocks2d.addCell(new Cell(new Point(x, y), Color.blue));
  }
    ...
    switch (command) {
      ...
      case "q": // multiple sinking pieces, test that each behaves correctly
        ...
        for (int x = 0, boardWidth = PlayField.getWidth(); x < boardWidth; x++) {
          ...
          for (int y = 16; y < PlayField.getHeight(); y++) if (x == 2) createCell(x, y, blocks2d);
